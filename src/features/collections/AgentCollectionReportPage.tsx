...
function exportPDF(collections: any[], fromDate: string, toDate: string, agentName: string, branding: any) {
  ...
  doc.setTextColor(30, 64, 175); doc.setFont('helvetica', 'bold'); doc.setFontSize(8);
  ...
  autoTable(doc, {
    ...
    styles: { fontSize: 7, cellPadding: 2, textColor: [55, 65, 81], lineColor: [229, 231, 235], lineWidth: 0.2 },
    ...
    willDrawCell: (data) => {
      if (data.section === 'body') {
        // Green amount column
        if (data.column.index === 6 && data.cell.text?.[0] && !collections[data.row.index]?.isReversed)
          doc.setTextColor(21, 128, 61);
        // Red for reversed
        if (data.column.index === 8 && data.cell.text?.[0]?.toLowerCase() === 'reversed')
